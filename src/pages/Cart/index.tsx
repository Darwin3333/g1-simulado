...
export function Cart() {
  ...
  function handleItemIncrement(itemId: string) {
    const newCartCoffee2 = coffeesInCart.map((coffe) =>
      coffe.id === itemId
        ? {
            ...coffe,
            quantity: coffe.quantity + 1,
            subTotal: coffe.price * coffe.quantity,
          }
        : coffe
    );

    setCoffeesInCart(newCartCoffee2);
  }

  function handleItemDecrement(itemId: string) {
    const newCartCoffee = coffeesInCart.map((coffe) =>
      coffe.id === itemId && coffe.quantity > 1
        ? { ...coffe, quantity: coffe.quantity - 1 }
        : coffe
    );

    setCoffeesInCart(newCartCoffee);
  }

  function handleItemRemove(itemId: string) {
    const newCartCoffee3 = coffeesInCart.filter(
      (coffee) => coffee.id !== itemId
    );

    setCoffeesInCart(newCartCoffee3);
  }
          ...
          {coffeesInCart.map((coffee) => (
            <Fragment key={coffee.id}>
              <Coffee>
                ...
                <aside>
                  R$ {(coffee.quantity * coffee.subTotal).toFixed(2)}
                </aside>
              </Coffee>
